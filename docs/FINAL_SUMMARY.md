...
| Firestore Rules | production 배포 완료 |
...
| Firestore Rules 추가 및 production 배포 | 완료 |
...
| 높음 | Rules 배포 후 전체 운영 smoke test | 일부 확인 필요 | 관리자/학생 주요 흐름을 production에서 재확인 |
...
관리자 권한 구조와 Firestore Rules production 적용은 크게 개선되었다. 다만 학생 인증, 사번 검증, 결과 무결성, Preview/Production 데이터 분리가 아직 남아 있어 production 운영 가능 상태로 보기는 어렵다.

## 다음 우선순위

1. owner가 승인한 사번 검증 방식을 확정한다.
2. 학생 공통 인증값을 제거한다.
3. Rules 배포 후 production 관리자/학생 기본 동작 점검을 완료한다.
