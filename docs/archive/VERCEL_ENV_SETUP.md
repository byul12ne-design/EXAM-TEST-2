...
Firebase CLI 로그인/로그아웃, Firestore Rules 배포, 배포 후 기본 동작 점검 절차는 `docs/operations/AUTH_AND_DEPLOY_OPERATION_GUIDE.md`를 기준으로 한다.
...
| Firestore Rules | Emulator 통과, production 배포 완료, 운영자 확인 기준 |
...
| Rules 배포 후 전체 smoke test 일부 미확인 | 관리자 CRUD와 학생 결과 저장/조회 추가 확인 필요 |
...
1. Firestore Rules production 배포 후 관리자 CRUD와 학생 결과 저장/조회 smoke test를 완료한다.
