...
export default function App() {
  ...
  // 💡 디자인 로딩 상태 관리 추가 (투박한 화면 방지)
  const [isStyleLoaded, setIsStyleLoaded] = useState(false);

  useEffect(() => {
    // Tailwind CSS 스크립트 주입 및 로딩 감지
    ...
    if ((window as any).tailwind) {
      setIsStyleLoaded(true); // 이미 로드되어 있으면 바로 통과
    } else {
  ...
  if (!isStyleLoaded) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', height: '100vh', backgroundColor: '#f8fafc', fontFamily: 'sans-serif' }}>
        <img src={APP_CONFIG.logoImageUrl} alt="Logo" style={{ height: '40px', marginBottom: '16px' }} />
        <p style={{ color: '#94a3b8', fontSize: '14px', fontWeight: 'bold' }}>디자인을 준비 중입니다...</p>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <div className="min-h-[100dvh] font-sans bg-slate-50 text-slate-900 flex flex-col relative animate-fade-in-up">
        ...
        <main className="p-4 sm:p-6 max-w-5xl mx-auto w-full flex-1 flex flex-col">
          ...
          {view === 'home' && !userProfile && (
            <div className="flex flex-col items-center gap-10 py-10 sm:py-20 text-center flex-1 justify-center">
              <div className="bg-white p-8 sm:p-12 rounded-[2.5rem] shadow-xl border w-full max-w-md space-y-8">
                <div>
                  <h2 className="text-3xl sm:text-4xl font-black text-slate-800 mb-2">교육 센터</h2>
                  <p className="text-slate-400 font-bold text-sm">사번을 입력하여 로그인하세요</p>
                </div>
                ...
                <div className="space-y-5">
                  <div className="flex items-center bg-slate-50 border-2 border-slate-100 rounded-2xl focus-within:border-blue-500 focus-within:ring-4 focus-within:ring-blue-50 transition-all overflow-hidden p-2">
                    <span className="pl-4 pr-2 font-black text-blue-600 text-xl">WN</span>
                    <input 
                      type="text" 
                      value={empIdInput} 
                      onChange={e => setEmpIdInput(e.target.value.replace(/[^0-9]/g, ''))} 
                      maxLength={8}
                      className="w-full bg-transparent p-3 pl-1 text-xl outline-none font-bold placeholder:font-normal text-slate-700 placeholder:text-slate-300" 
                  ...
                  {authMode === 'register' && (
                    <input 
                      ...
                      className="w-full bg-slate-50 border-2 border-slate-100 p-5 rounded-2xl text-xl outline-none focus:border-blue-500 focus-within:ring-4 focus-within:ring-blue-50 transition-all text-center font-bold"
                    />
                  )}
                  <button onClick={handleStudentAuth} className="w-full bg-slate-900 text-white font-black text-xl py-5 rounded-2xl shadow-xl hover:bg-blue-600 active:scale-95 transition-all mt-4">
                    {authMode === 'login' ? '입장하기' : '등록하고 입장하기'}
                  </button>
                </div>
              </div>
              <button onClick={() => setView('admin-login')} className="text-slate-300 hover:text-slate-500 text-sm font-bold transition-colors">⚙️ 관리자 모드 접속</button>
            </div>
          )}
          ...
          {view === 'admin-login' && (
            <div className="max-w-xs mx-auto py-20">
              ...
              <input type="password" value={adminPasswordInput} onChange={e => setAdminPasswordInput(e.target.value)} className="w-full border-2 p-5 rounded-2xl mb-4 text-center font-bold text-xl outline-none focus:border-slate-800" placeholder="비밀번호"/>
              <button onClick={handleAdminLogin} className="w-full bg-slate-800 text-white py-5 rounded-2xl font-bold text-xl hover:bg-slate-700 transition-colors">인증하기</button>
              <button onClick={() => setView('home')} className="w-full text-slate-400 text-sm mt-6 font-bold hover:text-slate-600">뒤로가기</button>
            </div>
          )}
            ...
            <div className="space-y-6">
              ...
              {adminTab === 'exams' && (
                <div className="space-y-4">
                  <button onClick={() => { resetAdminForm(); setView('admin-create'); }} className="w-full sm:w-auto bg-blue-600 text-white px-6 py-4 rounded-xl font-bold shadow-md hover:bg-blue-700 transition-colors">➕ 새로운 학습/퀴즈 세트 만들기</button>
                  <div className="grid gap-3">
          ...
          {view === 'student-entry' && (
            <div className="py-20 text-center space-y-6">
              ...
              <button onClick={startExam} className="bg-blue-600 text-white px-12 py-5 rounded-[2rem] font-black shadow-2xl text-xl hover:bg-blue-700 transition-all">과정 시작하기 👉</button>
            </div>
          )}
